...
#		ncurses-base=5.9+20140913-1+deb8u3 \
		libonig-dev \
		libpq-dev \
    && docker-php-ext-install -j$(nproc) iconv pdo_mysql pdo_pgsql mbstring xml tokenizer zip intl \
...
COPY develop/apache/ssl/ssl-cert-snakeoil.key /etc/ssl/private/
COPY develop/apache/ssl/ssl-cert-snakeoil.pem /etc/ssl/certs/
